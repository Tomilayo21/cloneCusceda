        ...
        <table className="min-w-full table-auto text-sm">
          ...
          <tbody>
            {paginatedUsers.map((user) => {
              ...
              const signupMethod = user.external_accounts?.[0]?.provider === "oauth_google" ? "Google" : "Email";

              return (
                <tr key={user.id} className="border-t hover:bg-gray-50">
                  <td className="px-2 py-2 border">
                    <img src={user.profile_image_url} alt="profile" className="w-8 h-8 md:w-10 md:h-10 rounded-full object-cover" />
                  </td>
