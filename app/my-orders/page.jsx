...
//     const dateKey = dayjs(order.date).format("dddd, DD MMMM YYYY");
//     if (!groups[dateKey]) groups[dateKey] = [];
//     groups[dateKey].push(order);
//     return groups;
//   }, {});

//   // Flatten for pagination (optional: paginate across all groups)
//   const allOrders = Object.entries(groupedOrders).flatMap(([_, group]) => group);
//   const totalPages = Math.ceil(allOrders.length / perPage);
//   const paginatedOrders = allOrders.slice((page - 1) * perPage, page * perPage);

//   // Re-group only paginated orders
//   const paginatedGrouped = paginatedOrders.reduce((groups, order) => {
//     const dateKey = dayjs(order.date).format("dddd, DD MMMM YYYY");
//     if (!groups[dateKey]) groups[dateKey] = [];
//     groups[dateKey].push(order);
//     return groups;
...
//             </div>
//           ) : (
//             <section className="grid gap-8 max-w-5xl text-sm">
//               {Object.entries(paginatedGrouped).map(([date, ordersForDate]) => (
...
// const MyOrders = () => {
//   const { currency, currentUser } = useAppContext();
//   const [orders, setOrders] = useState([]);
//   const [loading, setLoading] = useState(true);
//   const router = useRouter();
//   const [page, setPage] = useState(1);
...
//                     const method = paymentMethods.find(m => m.id === order.paymentMethod);
//                     return (
//                       <div key={order._id || index} className="bg-white dark:bg-gray-900 shadow-sm border rounded-xl p-5 space-y-5">
//                         {/* Render order preview, details, actions as before */}
//                         {/* ... you can reuse your existing JSX */}
//                       </div>
//                     );
//                   })}
//                 </div>
...
//           <div className="mt-8 flex justify-center items-center">
//             <div className="flex items-center gap-2 bg-white dark:bg-gray-900 shadow-sm border rounded-lg px-4 py-2">
//               <button disabled={page === 1} onClick={() => setPage(p => Math.max(p-1,1))} className="px-3 py-1 text-sm rounded bg-gray-200 dark:bg-gray-700 disabled:opacity-50">Prev</button>
//               {Array.from({ length: totalPages }).map((_, i) => (
//                 <button key={i} onClick={() => setPage(i+1)} className={`px-3 py-1 text-sm rounded ${page === i+1 ? "bg-orange-600 text-white" : "bg-gray-100 dark:bg-gray-800 hover:bg-orange-100"}`}>{i+1}</button>
//               ))}
//               <button disabled={page === totalPages} onClick={() => setPage(p => Math.min(p+1,totalPages))} className="px-3 py-1 text-sm rounded bg-gray-200 dark:bg-gray-700 disabled:opacity-50">Next</button>
//             </div>
//           </div>
//         )}
//       </main>
...
//   //     try {
//   //       const { data } = await axios.get("/api/order/list", {
//   //         headers: { "Authorization": `Bearer ${currentUser._id}` }, // send Mongo _id
//   //       });
...
//   // }, [currentUser, router]);
// useEffect(() => {
//   if (!currentUser?._id) return; // wait for Mongo _id

//   const fetchOrders = async () => {
//     try {
//       const { data } = await axios.get("/api/order/list", {
//         headers: { "Authorization": `Bearer ${currentUser._id}` },
//       });
...
//       <Navbar />
//       <main className="px-6 md:px-16 lg:px-32 py-6">
...
//           <div className="space-y-6">
//             {orders.map(order => (
//               <div key={order._id} className="bg-white dark:bg-gray-900 shadow-sm border rounded-xl p-5">
//                 <p><strong>Order ID:</strong> {order._id}</p>
//                 <p><strong>Status:</strong> {order.status}</p>
//                 <p><strong>Date:</strong> {dayjs(order.createdAt).format("DD MMM YYYY, hh:mm A")}</p>
//                 {/* Render items */}
//                 {order.items.map(item => (
//                   <div key={item.product._id} className="flex justify-between mt-2">
